...
### Added
- `brz mcp` runs a Model Context Protocol server over stdio. Speaks JSON-RPC 2.0; any LLM client (Claude Code, Cursor, etc.) can introspect via `tools/list` and drive a real Chrome browser via `tools/call` without writing YAML. Nine v1 tools: `browser_goto`, `browser_click`, `browser_type`, `browser_extract`, `browser_screenshot`, `browser_eval`, `browser_wait_for`, `browser_get_url`, `browser_session_info`. Single browser session per process, lazy-launched on first navigation, torn down on stdin EOF or `--idle-timeout` (default 5m). Tool calls are serialized through a single mutex around the rod browser handle. Implementation hand-rolls JSON-RPC framing — no new dependencies.
...
- `brz probe <url>` — interactive selector REPL. Opens a headed browser, navigates to the URL, and drops into a stdin loop where each line is evaluated as a CSS selector (default) or XPath (prefix `xpath:` or `--xpath`). Reports match count, prints a tag/attrs/text preview of the first matches, and highlights them in the live browser via an injected `outline: 3px solid red` stylesheet (cleared between queries). Reserved commands: `:q`, `:reload`, `:url <url>`, `:eval <js>`, `:mode css|xpath`, `:save <name>` (appends to `./selectors.yaml`), `:help`. Malformed selectors print a one-line `invalid selector: ...` and the prompt continues — the REPL never crashes. `--headless` flag suppresses the window and highlight injection for CI smoke-tests.
...
- Selector aliases. New top-level `aliases:` map binds short names to selector strings; reference them from any step as `${aliases.NAME}`. When a site changes one selector used in N places, the user fixes one alias instead of N step lines. `aliases_from:` directive loads aliases from external YAML files (each a flat `name: selector` map; supports `~`, absolute, and workflow-relative paths; files merge in declaration order, later files override earlier on key collision; inline `aliases:` always overrides). Alias-of-alias chains resolve transitively; cycles detected at parse time. Undefined alias references parse-error with a Levenshtein-based "did you mean" suggestion. Runtime errors include the alias name (and source file when external). Path resolution rejects symlinks that escape `$HOME`.
- `Executor.Browser()` accessor exposes the underlying `*rod.Browser` for advanced consumers (used by the MCP server, which manages its own page lifecycle).

### Fixed
- `launchForLogin` now uses `--remote-debugging-port=0` so the OS assigns a free port, eliminating collisions with other Chrome instances that happen to hold port 9222. Port is discovered from `<profileDir>/DevToolsActivePort` after Chrome starts (polled up to 5s); new `DebugPort()` getter exposes it to callers.
...
### Compatibility
- All additions are backwards compatible. Default behavior of `brz run` with no new flags is byte-identical. No YAML schema fields removed or renamed; new fields (`aliases`, `aliases_from`) are `omitempty`. No CLI flags removed. No exit codes changed.
